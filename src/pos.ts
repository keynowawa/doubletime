...
function renderProductModal() {
  ...
  return `<div class="modal-layer"><form class="modal-card editor-card" id="product-form">${modalHead(item ? 'edit product' : 'add product', 'changes only affect the pos menu.')}
    ...
    <div class="modal-split editor-actions ${item ? 'paired' : ''}">${item ? '<button type="button" class="danger-button" data-action="archive-product"><i data-lucide="archive"></i><span>archive product</span></button>' : '<span></span>'}<button class="modal-primary fit" type="button" data-editor-save="product"><span>save product</span><i data-lucide="check"></i></button></div>
  </form></div>`;
}

function renderAddonModal() {
  const item = editingModifier;
  return `<div class="modal-layer"><form class="modal-card compact" id="modifier-form">${modalHead(item ? 'edit add-on' : 'add an add-on', 'keep extras simple and quick to tap.')}<input type="hidden" name="id" value="${esc(item?.id || '')}"><label><span>add-on name</span><input name="name" value="${esc(item?.name || '')}" placeholder="e.g. oat milk" required></label><div class="two-fields"><label class="sku-field"><span>sku</span><input id="modifier-sku" name="sku" value="${esc(item?.sku || 'DT-ADD-')}" placeholder="e.g. DT-ADD-OAT" autocapitalize="characters" autocomplete="off" spellcheck="false" required><span class="sku-suggestions"><small>quick start</small><button type="button" data-sku-prefix="DT-ADD-" data-sku-target="modifier-sku">DT-ADD-</button></span></label><label><span>price</span><input name="price" type="number" min="0" step="1" value="${item?.price ?? ''}" placeholder="0" required></label></div><div class="modal-split editor-actions ${item ? 'paired' : ''}">${item ? '<button type="button" class="danger-button" data-action="archive-modifier"><i data-lucide="archive"></i><span>archive add-on</span></button>' : '<span></span>'}<button class="modal-primary fit" type="button" data-editor-save="modifier"><span>save add-on</span><i data-lucide="check"></i></button></div></form></div>`;
}
